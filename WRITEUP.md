...
\- \*\*IntentEngine:\*\* Parses the user-entered query, extracts main keywords, and applies hard structural filters (e.g., checking the `is\_public` state).

\- \*\*Scoring System:\*\* Calculates a relevance score for each candidate company based on keyword occurrences in key fields (`operational\_name`, `address`, `core\_offerings`, `target\_markets`, and `description`).
...
\- \*\*Synonym Mismatch:\*\* If a query specifies “freight transportation,” but a company description uses “freight transportation” with no overlapping keywords, the scoring system may underestimate or miss the match.
...
\- If a company profile lists popular keywords or industries in its description or bids for visibility in gaming searches, the scoring system could give it an inflated rank despite a lack of core competency.
